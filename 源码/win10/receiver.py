...
class CameraWorker(threading.Thread):
    ...
    def _detect_qr_simple(self, frame):
        """检测画面中的二维码"""
        # 策略1：原图检测（最快，优先）
        count = self._try_detect_multi(frame, frame)
        if count > 0:
            return count

        # 策略2：CLAHE 增强（屏幕二维码对比度偏低时备用）
        try:
    ...
    def _try_detect_multi(self, scan, draw_frame):
        ...
        elif self._detect_attempts > 0 and self._detect_attempts % 2000 == 0:
